...
def read_cpu_pkg_sensors():
    """One CPU socket's package power (W) via sensors PPT. None if unavailable."""
    try:
        out = subprocess.run(["sensors"], capture_output=True, text=True, timeout=5)
        m = PPT_RE.findall(out.stdout)        # one entry per socket
        if m:
            return float(m[0])                # first socket = "one CPU"
    except Exception:
        pass
    return None
...
def read_node_dcmi():
    """Whole-node power (W) from the BMC via ipmitool dcmi. 0.0 if unavailable."""
    try:
        cmd = shlex.split(IPMITOOL_CMD) + ["dcmi", "power", "reading"]
        out = subprocess.run(cmd, capture_output=True, text=True, timeout=8)
        m = DCMI_RE.search(out.stdout)
        if m:
...
if __name__ == "__main__":
    ...
    print(f"Serving on http://0.0.0.0:{PORT}  (GPU {GPU_INDEX}, power mode '{POWER_MODE}', "
