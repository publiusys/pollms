...
#   5. Waits for the model to load, then starts server_h200.py (proxy +
...
MODEL_FILE="${MODEL_FILE:-Qwen3.5-27B-Q5_K_M.gguf}"
...
for f in index.html server_h200.py; do
...
if screen -ls 2>/dev/null | grep -q "[.]$PROXY_SCREEN[[:space:]]"; then
  ...
  log "Starting server_h200.py in screen '$PROXY_SCREEN' (port $PROXY_PORT, power mode '$POWER_MODE') ..."
  screen -L -Logfile "$PROJECT_DIR/proxy.log" -dmS "$PROXY_SCREEN" \
         env GPU_INDEX="$GPU_INDEX" N_CPU_SOCKETS="$N_CPU_SOCKETS" \
             ...
             python3 "$PROJECT_DIR/server_h200.py"
fi
